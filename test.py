...
df = read_table(filepath, skiprows=1, delim_whitespace=True)
...
print(index)
index += round(5/deltat)

#check data
l = [index]
index += round(65/deltat)
l += [index]
index += round(5/deltat)
for i in range(18):
    l += [index + round((1 + i * 15)/deltat), round(index + (4 + i * 15)/deltat), round(index + (6 + i * 15)/deltat), round(index + (9 + i * 15)/deltat)]

dfs = [df.iloc[l[2 * i]:l[2 * i+1]] for i in range(37)]
plt.plot(dfs[3])
plt.show()



data = [[dft.checkdata(testindex, electrodeindex, dfs[testindex].iloc[:, electrodeindex], deltat) for electrodeindex in range(18)] for testindex in range(37)]
